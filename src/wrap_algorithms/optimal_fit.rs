...
pub fn wrap_optimal_fit<'a, 'b, T: Fragment>(
    ...
    // Accumulated fragment widths as a u64. With each fragment using
    // u32, we can add up about 2**32 fragments before overflowing.
    let mut width: u64 = 0;
    widths.push(width);
...
mod tests {
    ...
    fn wrap_string_longer_than_u32() {
        ...
        let penalties = &OptimalFit {
            overflow_penalty: 1_000_000_000,
            ..OptimalFit::default()
        };

        assert_eq!(
            wrap_optimal_fit(&words, &[3_500_000_000], &penalties),
            &[
                vec![Word(1_000_000_000), Word(2_000_000_000)],
                vec![Word(3_000_000_000)],
                vec![Word(4_000_000_000)],
            ]
        );
    }
}
