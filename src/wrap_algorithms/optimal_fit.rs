...
pub struct OptimalFit {
    ...
    /// let wrapped = wrap_optimal_fit(&fragments, &[length - 1.0], &penalties);
    /// assert_eq!(wrapped, vec![&[Word::from(short), Word::from(&long)]]);
    ///
    ...
    /// let wrapped = wrap_optimal_fit(&fragments, &[length - 2.0], &penalties);
...
impl WrapAlgorithm for OptimalFit {
    ...
    fn wrap<'a, 'b>(&self, words: &'b [Word<'a>], line_widths: &'b [usize]) -> Vec<&'b [Word<'a>]> {
        ...
        wrap_optimal_fit(words, &f64_line_widths, self)
    }
}
...
/// # Panics
///
/// In case of an overflow during the cost computation, a panic is
/// triggered. Overflows happens when fragments have infinite widths
/// or if the line width is to large that the gaps widths in the range
/// of approximately 1e170:
///
/// ```should_panic
...
/// }
///
/// // 1e155 * 1e155 = 1e310, which is larger than f64::MAX. The
/// // result is an overflow.
/// wrap_optimal_fit(&[Word(0.0), Word(0.0)], &[1e155], &OptimalFit::default());
/// ```
///
/// Fragment widths which fit inside an `u64` are safe. This means
/// that fragments derived from a `&str` cannot cause overflows.
///
/// **Note:** Only available when the `smawk` Cargo feature is
/// enabled.
